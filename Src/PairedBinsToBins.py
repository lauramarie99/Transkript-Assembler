...
def fromPairedBinsToBins(pairedBins, Bins, graph, Exons:list):
    ...
    for bin in pairedBins:                                                                                                              # Read Bins from PairedBins
        if checkPairedBins (bin) == False:                                                                                              # Check potential order mistakes
            continue
        rightExons = bin.rightExons
        leftExons = bin.leftExons

        if rightExons [len(rightExons)-1] < leftExons[0]:
...
#   V   # 13. left 1,2,3    right 1
        ...
        while (rightExons[0] < leftExons[len(leftExons)-1]) and len(rightExons)>0:                                                            # As long as first_right_exon is smaller than last_left_exon                                                                                                                              

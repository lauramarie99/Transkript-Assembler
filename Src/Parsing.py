...
def write_valid_gtf_entry(f, chrom, strand, exons, transcript, geneId, transcriptId):
    ...
    f.write(chrom + "\tFortgMethoden\ttranscript\t" + str(exons[transcript[0]].leftPos) + "\t" + str(exons[transcript[
        -1]].rightPos) + "\t0\t" + strand + '\t.\tgene_id "' + geneId + '"; transcript_id "' + transcriptId + '";\n')
    ...
    for ti in transcript[1:]:
        if exons[ti].leftPos == right + 1:  # adjacent, extend
            ...
            f.write(chrom + "\tFortgMethoden\texon\t" + str(left) + "\t" + str(
                right) + "\t0\t" + strand + '\t.\tgene_id "' + geneId + '"; transcript_id "' + transcriptId + '";\n')
...
with open("/home/tezie/info2/Test.graph") as f:
    ...
    while not fileEndReached:
        ...
        write_valid_gtf_entry(dummyf, Chromosome, Strand, Exons, transcript, "Gene" + str(dummyGeneCounter),
                              "Transcript" + str(dummyGeneCounter) + ".1")

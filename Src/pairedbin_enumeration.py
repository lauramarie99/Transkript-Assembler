...
def group_pairs(pairedbins):
    ...
    for pairedbin in pairedbins:
        ...
        # check if bins are valid: if left and right exons share exons, the transitions should be the same
        j = 0
        for i in range(0,len(left)):
            if left[i] == right[j]:
                j += 1
        ...
        # Continue with next paired bin if current bin is invalid
        if invalid == True:
            continue
        
        # Continue if left or right exons are empty
        ...
        # Continue if paired bin doesn't provide new information
        if len(right) == 1 and len(left) == 1 and left[0] + 1 == right[0]:
            continue
        if len(right) == 1 and right[0] in left:
            continue
        
        # Add current paired bin to dictionary
